// ====================== Canvas Coloring (2-layer, AA lines, mobile/desktop) ======================

const canvas = document.getElementById("canvas");
const ctx = canvas.getContext("2d");

// chặn scroll/zoom khi vẽ trên mobile
if (canvas && canvas.style) canvas.style.touchAction = "none";

// ---------- Config chuẩn hoá & khử răng cưa ----------
const T_HIGH = 165;          // < T_HIGH => chắc chắn "đen" (nằm trong nét)
const T_LOW  = 220;          // > T_LOW  => chắc chắn "trắng"
const DILATE_RADIUS = 0;     // nở nét 0..2 để bịt khe cực nhỏ (0: tắt)

const AA_BLUR_PX = 1.2;      // độ mờ ở không gian upsample
const UPSCALE_LIMIT_MP = 12_000_000; // giới hạn số pixel sau upsample (an toàn mobile)

// ---------- Fill “kín mép” (để chắc chắn không còn viền trắng) ----------
const WHITE_LUMA = 238;      // pixel >= ngưỡng này coi như gần-trắng
const FILL_GROW_RADIUS = 1;  // nở mép vùng tô (0..2). 1 là đủ an toàn.
...
// 2 LAYER: color (vẽ/tô) và line (nét trong suốt phủ lên)
...
// mặt nạ nét nhị phân để chặn flood-fill tràn vùng
let lineMask = null;

// lưu điểm trước đó để nội suy brush
let lastPt = null;
...
function getAAScale(w, h) {
  const prefer = isMobile() ? 3 : 5;
  const maxScale = Math.max(2, Math.floor(Math.sqrt(UPSCALE_LIMIT_MP / (w * h))));
  return Math.min(prefer, maxScale);
}
...
function setCurrentColor(hex) {
  ...
  currentColor = /^0{6}$/i.test(val) ? "#111111" : "#" + val.toUpperCase(); // không dùng đen thuần
}
...
document.getElementById("textModeBtn").addEventListener("click", () => { mode = "text"; updateModeButtons(); addTextBoxCentered(); });
...
document.getElementById("brushSizeSelect").addEventListener("change", function () { brushSize = parseFloat(this.value); });
...
// ----------------- Brush / Eraser (chỉ vẽ lên colorLayer) -----------------
function strokeFromTo(x0, y0, x1, y1, radius, rgba, isErase=false) {
  const dx = x1 - x0, dy = y1 - y0;
  const dist = Math.hypot(dx, dy);
  if (dist === 0) { paintCircleOnColor(x1, y1, radius, rgba, isErase); return; }
  const STEP_FACTOR = isMobile() ? 0.6 : 0.5;
  const step = Math.max(1, radius * STEP_FACTOR);
  const n = Math.ceil(dist / step);
  for (let i = 1; i <= n; i++) {
    const t = i / n;
    paintCircleOnColor(x0 + dx * t, y0 + dy * t, radius, rgba, isErase);
  }
}
function drawAt(e) {
  ensureInitialized();
  ...
  if (!lastPt) { paintCircleOnColor(x, y, brushSize, rgba, isErase); lastPt = { x, y }; }
  else { strokeFromTo(lastPt.x, lastPt.y, x, y, brushSize, rgba, isErase); lastPt = { x, y }; }
  redrawComposite();
}
// events
canvas.addEventListener("mousedown", (e) => { if (mode==="brush"||mode==="eraser"){ isDrawing=true; saveState(); lastPt=null; drawAt(e);} });
canvas.addEventListener("mousemove", (e) => { if (isDrawing && (mode==="brush"||mode==="eraser")) drawAt(e); });
...
canvas.addEventListener("touchstart",(e)=>{ if(mode==="brush"||mode==="eraser"){ isDrawing=true; saveState(); lastPt=null; drawAt(e); e.preventDefault();}}, {passive:false});
canvas.addEventListener("touchmove", (e)=>{ if(isDrawing&&(mode==="brush"||mode==="eraser")){ drawAt(e); e.preventDefault();}}, {passive:false});
canvas.addEventListener("touchend",  ()=>{ isDrawing=false; lastPt=null; });

// ----------------- Fill (tô lên colorLayer, chặn bởi lineMask, nở mép) -----------------
...
function floodFillColorLayer(x, y, fillColor) {
  ...
  let imageData;
  try { imageData = colorCtx.getImageData(0,0,w,h); }
  catch(err){ console.error(err); alert("Không thể tô màu (CORS)."); return; }
  const data = imageData.data;

  if (isLinePixel(x,y,w,h)) return; // click ngay trên nét -> bỏ
  ...
  const stack = [[x,y]];
  const match = (cx,cy,i) => {
    if (isLinePixel(cx,cy,w,h)) return false;
  ...
  while(stack.length){
    ...
    const i=(cy*w+cx)*4, vi=cy*w+cx;
    if (visited[vi]) continue;
    visited[vi]=1;
    if (!match(cx,cy,i)) continue;

    data[i]=fillColor[0]; data[i+1]=fillColor[1]; data[i+2]=fillColor[2]; data[i+3]=255;
    stack.push([cx-1,cy],[cx+1,cy],[cx,cy-1],[cx,cy+1]);
  }

  // nở mép vào các pixel gần-trắng ngay sát đường
  if (FILL_GROW_RADIUS>0) growFillEdges(imageData, visited, fillColor);

  colorCtx.putImageData(imageData,0,0);
}
function growFillEdges(imageData, visited, fillColor){
  ...
  for(let y=0;y<h;y++){
    for(let x=0;x<w;x++){
      ...
      for(const [dx,dy] of N8){
        ...
        if (isLinePixel(nx,ny,w,h)) continue;
        const nvi=ny*w+nx;
        if(visited[nvi]) continue;
        const i=(ny*w+nx)*4;
        if(luma(i)>=WHITE_LUMA){
          data[i]=fillColor[0]; data[i+1]=fillColor[1]; data[i+2]=fillColor[2]; data[i+3]=255;
          visited[nvi]=1;
        }
      }
...
// ----------------- Vẽ tròn lên colorLayer -----------------
function paintCircleOnColor(x, y, radius, rgba, isErase=false) {
  const w=colorCanvas.width, h=colorCanvas.height;
  ...
  catch(err){ console.error(err); alert("Không thể vẽ (CORS)."); return; }
  const d=imageData.data, rr=radius*radius;

  for(let yy=y0; yy<=y1; yy++){
    for(let xx=x0; xx<=x1; xx++){
      const dx=xx-x, dy=yy-y;
      if(dx*dx+dy*dy>rr) continue;
      // vẽ tự do dưới line; nếu muốn tuyệt đối không chạm nét, bỏ comment dưới:
      // if (isLinePixel(xx,yy,w,h)) continue;

      const i=((yy-y0)*(x1-x0+1)+(xx-x0))*4;
      if (isErase){ d[i]=255; d[i+1]=255; d[i+2]=255; d[i+3]=255; }
      else { d[i]=rgba[0]; d[i+1]=rgba[1]; d[i+2]=rgba[2]; d[i+3]=255; }
    }
  }
  colorCtx.putImageData(imageData,x0,y0);
}

// ----------------- Undo / Redo (snapshot colorLayer) -----------------
function saveState() {
  ensureInitialized();
...
document.getElementById("downloadBtn").addEventListener("click", () => {
  ...
  logo.onload = () => {
    ...
    // 3) logo
    const logoHeight = 30;
    const s = logoHeight / logo.height;
    const logoWidth = logo.width * s;
...
// (Các hàm drag/resize/rotate/select textbox giữ nguyên như bản trước)
function makeTextBoxDraggable(box){ /* ... giữ nguyên code cũ của bạn ... */ }
function enableResize(textBox){ /* ... giữ nguyên code cũ của bạn ... */ }
function applyTransform(box){ const angle=parseFloat(box.dataset.rotation||"0"); const sx=parseFloat(box.dataset.scaleX||"1"); const sy=parseFloat(box.dataset.scaleY||"1"); box.style.transform=`rotate(${angle}deg) scale(${sx}, ${sy})`; }
function enableRotate(textBox){ /* ... giữ nguyên code cũ của bạn ... */ }
function handleTextBoxSelection(e){ const box=e.target.closest(".text-box"); if(box){ currentTextBox=box; const content=currentTextBox.querySelector(".text-content"); if(content) content.style.color=currentColor; } }
...
document.getElementById("boldBtn").addEventListener("click", () => { if(currentTextBox){ const c=currentTextBox.querySelector(".text-content"); const isBold=c.style.fontWeight==="bold"; c.style.fontWeight=isBold?"normal":"bold"; }});
document.getElementById("fontSelect").addEventListener("change", (e) => { if(currentTextBox){ const c=currentTextBox.querySelector(".text-content"); c.style.fontFamily=e.target.value; }});
...
// ======================  Chuẩn hoá ảnh nguồn & dựng layer  ======================

// đảm bảo canvas hiển thị & 2 layer có size và màu nền trắng mặc định
function ensureInitialized() {
  ...
  if (colorCanvas.width !== canvas.width || colorCanvas.height !== canvas.height) {
  ...
  if (lineCanvas.width !== canvas.width || lineCanvas.height !== canvas.height) {
...
// composite 2 layer lên canvas hiển thị
...
// Vẽ ảnh vào layer và dựng nét mượt + mask
function loadImageToLayers(image) {
  // scale ảnh nguồn cho vừa sức thiết bị
  ...
  // vẽ ảnh xuống tạm để đọc pixel
  const tmp = document.createElement("canvas");
  tmp.width = w; tmp.height = h;
  const tctx = tmp.getContext("2d");
  tctx.imageSmoothingEnabled = true;
  if ('imageSmoothingQuality' in tctx) tctx.imageSmoothingQuality = 'high';
  tctx.drawImage(image, 0, 0, srcW, srcH, 0, 0, w, h);

  let id;
  try { id = tctx.getImageData(0,0,w,h); }
  catch(err){ console.error(err); alert("Không thể xử lý ảnh (CORS)."); return; }
  const d = id.data;

  // 1) phân loại: hardBlack / hardWhite
  ...
  // 2) hysteresis: vùng xám kề đen -> nhập vào đen
  ...
  // 4) lưu mask để chặn tràn vùng khi fill
  lineMask = outBlack;

  // 5) tạo lineLayer mượt (đen, nền alpha)
  renderLineLayerFromMask(outBlack, w, h, getAAScale(w,h));

  // 6) reset colorLayer = trắng
...
// === Dựng lineLayer: upsample (no smoothing) -> blur -> downsample -> chuyển grayscale -> alpha ===
function renderLineLayerFromMask(mask, w, h, scale=3){
  ...
  // chuyển grayscale -> alpha (đen đậm => alpha cao)
  let lid;
  ...
  for(let i=0;i<ld.length;i+=4){
    ...
    ld[i+3]=Math.max(0, Math.min(255, 255 - lum)); // trắng -> alpha 0, đen -> alpha 255
  }
